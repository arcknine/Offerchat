@Offerchat.module "NavigationApp.Show", (Show, App, Backbone, Marionette, $, _) ->
  ...
  class Show.Nav extends App.Views.ItemView
    ...
    serializeData: ->
      ...
      is_pro_acct: if ["PRO", "PROTRIAL", "AFFILIATE", "PROYEAR"].indexOf(@options.model.get("plan_identifier")) is -1 then false else true
      not_basic: @options.model.get("plan_identifier") isnt "BASIC" and @options.model.get("plan_identifier") isnt "BASICYEAR"
